...
import com.zjs.customer.danniao.service.common.StandardGetService;
...
public abstract class AbstractStandardDockingService {
	private static final Logger logger = LoggerFactory.getLogger(AbstractStandardDockingService.class);
	 ...
	 * 外部接口调用服务
	 */
	abstract class ExternalInterfaceInvocationService {
		/**
		 * 具体处理模块
		 *
		 * @param uuid
		 ...
		 * @return 处理后记录Edi_Log结果
		 */
		protected Edi_Log orderStatusHandler(String uuid, CommonMsgModel cmm, String invokedInterfaceUrl,
			...
			if (cmm != null && cmm.getNd() != null) {
				return getEdiLogRespResultByMap(uuid, cmm.getNd(), invokedInterfaceUrl, interfaceName,
				                                stringObjectMultiValueMap, httpHeaders, requestTypeIsGet);
			} else {
				return getEdiLogRespResultByMap(uuid, invokedInterfaceUrl, interfaceName,
				                                stringObjectMultiValueMap, httpHeaders, requestTypeIsGet);
			}
		}
		 ...
		 * @return 调用结果
		 */
		private String getInvocationInterfaceResult(String uuid, String invokedInterfaceUrl, String interfaceName,
		                                            MultiValueMap<String, Object> linkedMultiValueMap,
		                                            HttpHeaders httpHeaders, boolean requestTypeIsGet) throws
		                                                                                               RuntimeException {
		 ...
		 */
		public Edi_Log getEdiLogRespResultByMap(String uuid, String node,
		                                        String invokedInterfaceUrl, String interfaceName,
		                                        MultiValueMap<String, Object> stringObjectMultiValueMap,
		                                        HttpHeaders httpHeaders, boolean requestTypeIsGet) {
			if (stringObjectMultiValueMap != null) {
				try {
					...
					if (StringUtils.isNotEmpty(invocationInterfaceResult) && judgeResult(uuid,
						...
						logger.info("{} 调用状态回传接口返回成功结果= {}", uuid, invocationInterfaceResult);
						return edilogtrue("当前正常状态: nd= " + node + "请求数据为=" + StringEscapeUtils.unescapeJava(JsonUtils.beanToJsonGsonImpl(stringObjectMultiValueMap)), "接口返回成功结果:" + invocationInterfaceResult);
					} else {
						logger.warn("{} 调用状态回传接口返回失败结果= {}", uuid, invocationInterfaceResult);
						return edilogfalse("当前失败状态: nd= " + node + "请求数据为=" + StringEscapeUtils.unescapeJava(JsonUtils.beanToJsonGsonImpl(stringObjectMultiValueMap)), "接口返回失败结果:" + invocationInterfaceResult, null);
					}
				} catch (Exception e) {
					//异常
			...
			return edilogfalse("当前错误状态: nd= " + node + "请求数据为空! ", "未调用接口! ", null);
		}
		 ...
		 * @return eid_log 日志,是否需要重推数据
		 */
		public Edi_Log getEdiLogRespResultByMap(String uuid, String invokedInterfaceUrl, String interfaceName,
		                                        MultiValueMap<String, Object> stringObjectMultiValueMap,
		                                        HttpHeaders httpHeaders, boolean requestTypeIsGet) {
			if (stringObjectMultiValueMap != null) {
				try {
					...
					if (StringUtils.isNotEmpty(invocationInterfaceResult) && judgeResult(uuid,
						...
						return edilogtrue("请求数据为=" + StringEscapeUtils.unescapeJava(JsonUtils.beanToJsonGsonImpl(stringObjectMultiValueMap)), "接口返回成功结果:" + invocationInterfaceResult);
					} else {
						logger.warn("{} 调用{}接口返回失败结果= {}", uuid, interfaceName, invocationInterfaceResult);
						return edilogfalse("请求数据为=" + StringEscapeUtils.unescapeJava(JsonUtils.beanToJsonGsonImpl(stringObjectMultiValueMap)), "接口返回失败结果:" + invocationInterfaceResult, null);
					}
				} catch (Exception e) {
					//异常
					logger.warn("{} 调用{}接口返回异常结果! 异常原因: {}", uuid, interfaceName, e);
					return edilogfalse("请求数据为=" + StringEscapeUtils.unescapeJava(JsonUtils.beanToJsonGsonImpl(stringObjectMultiValueMap)), "接口返回异常结果:", e.toString());
				}
			}
			//	 失败 or 错误
			logger.warn("{} 调用{}接口参数为空! ", uuid, interfaceName);
			return edilogfalse("请求数据为空! ", "未调用接口! ", null);
		}
	}
	 ...
	 * 内部接口调用服务
	 */
	abstract class InternalInterfaceInvocationService {
		 ...
		 * @return 处理后记录Edi_Log结果
		 */
		protected Edi_Log orderStatusHandler(String uuid, String invokedInterfaceUrl,
		                                     String interfaceName, String jsonData,
		                                     String clientFlag, String verifyData) {
			if (StringUtils.isEmpty(jsonData)) {
				jsonData = "{}";
			}
			return getEdiLogRespResultByJson(uuid, invokedInterfaceUrl, interfaceName,
			                                 jsonData, clientFlag, verifyData);
		}
		 ...
		 */
		private String getInvocationInterfaceResult(String uuid, String invokedInterfaceUrl, String interfaceName,
		                                            String jsonData, String clientFlag, String verifyData) throws
		                                                                                                   RuntimeException {
			String responseResult = "";
			try {
		...
		public Edi_Log getEdiLogRespResultByJson(String uuid, String invokedInterfaceUrl, String interfaceName,
			...
			if (jsonData != null) {
				try {
					...
					//异常
					logger.warn("{} 调用{}接口返回异常结果! 异常原因: {}", uuid, interfaceName, e);
					return edilogfalse("请求数据为=" + StringEscapeUtils.unescapeJava(JsonUtils.beanToJsonGsonImpl(jsonData)),
					                   "接口返回异常结果:", e.toString());
				}
			}
			//	 失败 or 错误
