...
public class TestCompletableFuture {
	...
	public static void main(String[] args) {
		try {
			String s1 =
					CompletableFuture.supplyAsync(() -> "Hello").thenApply(i -> i + 1).thenApply(s -> s + " World").thenApply(String::toUpperCase).get(10, TimeUnit.MILLISECONDS);
			...
			CompletableFuture.supplyAsync(() -> 1).thenApply(i -> i + 1).thenApply(i -> i + i).thenApply(s -> s + " " + "World").whenCompleteAsync((r, throwable) -> {
				System.out.println(r);
			}).exceptionally(Throwable::toString).join();
