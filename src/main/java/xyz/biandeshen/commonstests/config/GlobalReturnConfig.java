		...
		public void handleException(HttpServletRequest request, HttpServletResponse response, Exception e) throws IOException {
			...
			if (e instanceof GlobalException) {
				...
				globalResult = new GlobalResult.Builder(globalException.getCode() == null ?
				                                        ...
				                                        "调用接口" + request.getRequestURI().toString() + "失败，" + message).data(globalException.getObj()).build();
				//logger.warn("{} 宅急送接口异常响应结果:  {}，异常原因: {}", globalException.getUuid(), globalResult.toString(),
				//            globalException.getMessage());
				logger.warn("{} 宅急送接口异常响应结果:  {}，异常原因: {}", globalException.getUuid(), globalResult.toString(),
				            e);
			} else {
				//系统异常
				globalResult = new GlobalResult.Builder(CommonResponseEntity.S0006.getCode(), "系统内部错误!").build();
			...
			response.getWriter().println(UnicodeBackslashUtils.unicodeToCn(globalResult.toString()));
		}
	}
}
